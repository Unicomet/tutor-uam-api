...
public class RegistrationService {
    ...
    public void saveOrUpdateRegistratrion(String email, RegistrationRequest request) {
        ...
        if (Objects.equals(request.getRoleTutorship().toString(), "Asesorado")) {
            ...
            Tutor tutor = Tutor.builder().user(user).degree(request.getStudyField()).description(request.getDescription()).studyField(request.getStudyField()).tutorshipPlace(request.getTutorshipPlace()).build();
